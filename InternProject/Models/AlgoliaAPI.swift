...
class AlgoliaAPI: NSObject {
    ...
    func searchPosts(for searchString: String, completion: @escaping ([Post]) -> Void) {
        // These are the post attributes we want to search for. This eliminates solutions/replies keywords from showing up
        let settings = ["searchableAttributes": [
            "ownerName",
            "postDescription",
            "postTitle",
            "postTags"
            ]]
        postIndex?.setSettings(settings, completionHandler: { [weak self] (content, error) -> Void in
            if error != nil {
